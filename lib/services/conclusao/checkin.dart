...
class enviacheckin {
  enviar() async {
    ...
    for (int i = 0; i < idcheckin.length; i++) {
      ...
      data =
          '$data{"id": ${idcheckin[i]},"descricao": "${nomescheckin[i]}","situacaoAntes": "${itenscheckin[i]}" $obs}$virgula';
    }
    //print(data);
    if (obsadc == null) {
      obsadc = "";
    ...
    String body =
        '{"itens": [$data],"localGps": "$latitude,$longitude","observacao": "$obsadc","etapa": "SERVICO_INICIADO"}';
    final headers = {
